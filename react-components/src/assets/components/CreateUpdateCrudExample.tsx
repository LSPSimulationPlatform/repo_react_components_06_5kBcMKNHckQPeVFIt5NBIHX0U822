...
export default function CreateUpdateCrudExample({
  ...
  countryOptions, // - Array of dropdown options for country selection
  ...
  >
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>  
      ...
      <Input
        label="Surname" // - Field label for accessibility and user guidance
        placeholder="Enter surname" // - Hint text to guide user input
        value={formData.surname} // - Controlled input value from form state
        handleInputChange={handleInputChange('surname')} // - Handler for surname field changes using curried function
        required // - Mark field as required with red asterisk indicator
      />  

      <Input
        label="Email" // - Field label for accessibility and user guidance
        placeholder="Enter email" // - Hint text to guide user input
        value={formData.email} // - Controlled input value from form state
        handleInputChange={handleInputChange('email')} // - Handler for email field changes using curried function
        required // - Mark field as required with red asterisk indicator
      />  

      <SelectBox
        label="Country" // - Field label for dropdown selection
        options={countryOptions} // - Array of selectable country options
        value={formData.country} // - Currently selected country value
        handleSelectChange={handleSelectChange('country')} // - Handler for country selection changes
        placeholder="Select a country" // - Hint text when no option is selected
        required // - Mark field as required with red asterisk indicator
      />  
      ...
      <SelectBox
        ...
        handleSelectChange={handleSelectChange('category')} // - Handler for category selection changes
