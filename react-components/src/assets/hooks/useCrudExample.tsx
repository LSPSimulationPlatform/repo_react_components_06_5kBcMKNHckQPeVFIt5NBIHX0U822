...
export default function useCrudExample() {
  ...
  ]; // - Static dropdown options for country field, each with display label and internal value
  ...
  const handleSelectChange = (field: string) => (value: string) => {
    setFormData(prev => ({ ...prev, [field]: value })); // - Updates specific field in formData using dynamic key, preserving other fields
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.surname.trim()) {
      message.error('Surname is required'); // - Shows error toast if surname field is empty or only whitespace
      return; // - Exit early to prevent form submission
    }
    if (!formData.email.trim()) {
      message.error('Email is required'); // - Shows error toast if email field is empty or only whitespace
      return; // - Exit early to prevent form submission
    }
    if (!formData.country) {
      message.error('Country is required'); // - Shows error toast if no country is selected
      return; // - Exit early to prevent form submission
    }
  ...
  const handleDelete = (record: any) => {
    ...
    if (editingRecord?.id === record.id) {
      ...
      setFormData({
        ...
        category: '',
        description: ''
      }); // - Reset form fields to prevent editing deleted record
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      dataIndex: 'surname',
      key: 'surname',
      sorter: (a, b) => a.surname.localeCompare(b.surname), // - Alphabetical sorting for surname column using locale-aware comparison
    },
    {
      title: 'Email',
      dataIndex: 'email',
      key: 'email',
      sorter: (a, b) => a.email.localeCompare(b.email), // - Alphabetical sorting for email column using locale-aware comparison
    },
    {
      title: 'Country',
      dataIndex: 'country',
      key: 'country',
      filters: countryOptions.map(opt => ({ text: opt.label, value: opt.value })), // - Create filter options from countryOptions array
      onFilter: (value, record) => record.country === value, // - Filter function to show only records matching selected country
    },
    {
      title: 'Category',
      dataIndex: 'category',
    ...
    },
    {
      ...
      dataIndex: 'createdAt',
      key: 'createdAt',
      render: (date: Date) => new Date(date).toLocaleDateString(), // - Format Date object to readable string using browser locale
      sorter: (a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime(), // - Sort by timestamp for chronological ordering
    },
  ...
  return {
    ...
    countryOptions, // - Static array of country options for dropdown component
